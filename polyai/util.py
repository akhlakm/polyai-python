...
def default_api_key() -> str:
    if polyai.api_key_path:
        with open(polyai.api_key_path, "rt") as k:
            api_key = k.read().strip()
            if not api_key.startswith("pl-"):
                raise ValueError(f"Malformed API key in {polyai.api_key_path}.")
            return api_key
    elif polyai.api_key is not None:
        return polyai.api_key
    else:
